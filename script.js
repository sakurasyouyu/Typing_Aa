...
const $=selector=>document.querySelector(selector);const prompt=$("#prompt"),japanese=$("#japanese"),box=$("#typebox"),hint=$("#hint"),bar=$("#bar"),scoreElement=$("#score"),wpm=$("#wpm"),accuracy=$("#accuracy"),timer=$("#timer"),modal=$("#modal");
const SESSION_SECONDS=60;
let mode="easy",letterCase="lower",passage=null,text="",targets=[],typed=[],attempts=0,correctTotal=0,misses=0,started=0,clock,last="",mistakeTimer=null;
...
function elapsedSeconds(){return started?Math.min(SESSION_SECONDS,Math.floor((Date.now()-started)/1000)):0}
function remainingSeconds(){return SESSION_SECONDS-elapsedSeconds()}
function score(){return (correctTotal-misses)*10}
function update(){const elapsed=elapsedSeconds(),totalAttempts=correctTotal+misses;scoreElement.textContent=score();accuracy.textContent=totalAttempts?Math.round(correctTotal/totalAttempts*100):100;wpm.textContent=started&&elapsed?Math.round(correctTotal/5/(elapsed/60)):0;timer.textContent=remainingSeconds();bar.style.width=typed.length/text.length*100+"%";if(started&&remainingSeconds()<=0)finish()}
function begin(){if(started)return;started=Date.now();hint.classList.add("hide");clock=setInterval(update,1000)}
function finish(){clearInterval(clock);scoreElement.textContent=score();$("#rs").textContent=score();$("#rw").textContent=wpm.textContent;$("#ra").textContent=accuracy.textContent+"%";$("#rm").textContent=misses;setTimeout(()=>{modal.hidden=false;$("#again").focus()},250)}
function applyLetterCase(value){return letterCase==="upper"?value.toUpperCase():value.toLowerCase()}
function generateTargets(base){base=base.replace(/\s+/g,"");const patterns=[["cha","tya"],["chu","tyu"],["cho","tyo"],["chi","ti"],["shi","si"],["tsu","tu"],["ja","zya"],["ju","zyu"],["jo","zyo"],["ji","zi"]],results=new Set();function walk(index,current){if(index>=base.length){results.add(current);return}for(const group of patterns){const hit=group.find(value=>base.startsWith(value,index));if(hit){group.forEach(value=>walk(index+hit.length,current+value));return}}const character=base[index],next=base[index+1]||"";if(character==="n"&&!/[aiueony]/.test(next))walk(index+1,current+"nn");walk(index+1,current+character)}walk(0,"");return [...results].map(applyLetterCase)}
...
function nextPassage(){choose();targets=generateTargets(passage.romaji);text=targets[0];japanese.innerHTML=passage.html;typed=[];bar.style.width=0;draw()}
function reset(){clearInterval(clock);clearTimeout(mistakeTimer);correctTotal=0;misses=0;attempts=0;started=0;modal.hidden=true;box.classList.remove("mistake");hint.classList.remove("hide");scoreElement.textContent=0;wpm.textContent=0;accuracy.textContent=100;timer.textContent=SESSION_SECONDS;nextPassage();box.focus()}
document.addEventListener("keydown",event=>{if(!modal.hidden||event.ctrlKey||event.metaKey||event.altKey)return;if(event.key==="Escape"){reset();return}if(event.key==="Backspace"){event.preventDefault();if(typed.length){typed.pop();correctTotal=Math.max(0,correctTotal-1)}setTextForInput();draw();update();return}if(event.key===" "){event.preventDefault();return}if(event.key.length!==1||remainingSeconds()<=0)return;event.preventDefault();begin();const key=applyLetterCase(event.key),nextInput=typedText()+key;if(targets.some(target=>target.startsWith(nextInput))){attempts++;correctTotal++;typed.push(key);setTextForInput(nextInput);box.classList.remove("mistake");draw();update();if(targets.includes(nextInput))nextPassage();return}attempts++;misses++;showMistake();update()});
