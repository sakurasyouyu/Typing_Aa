...
let mode="easy",letterCase="lower",passage=null,text="",typed=[],started=0,clock,last="";
...
function update(){const elapsed=seconds(),correct=typed.filter((character,index)=>character===text[index]).length;accuracy.textContent=typed.length?Math.round(correct/typed.length*100):100;wpm.textContent=started?Math.round(correct/5/(elapsed/60)):0;timer.textContent=elapsed;bar.style.width=typed.length/text.length*100+"%"}
...
function reset(){clearInterval(clock);choose();text=applyLetterCase(passage.romaji);japanese.innerHTML=passage.html;typed=[];started=0;modal.hidden=true;hint.classList.remove("hide");wpm.textContent=0;accuracy.textContent=100;timer.textContent=0;bar.style.width=0;draw();box.focus()}
document.addEventListener("keydown",event=>{if(!modal.hidden||event.ctrlKey||event.metaKey||event.altKey)return;if(event.key==="Escape"){reset();return}if(event.key==="Backspace"){event.preventDefault();typed.pop();draw();update();return}if(event.key.length!==1||typed.length===text.length)return;event.preventDefault();begin();typed.push(event.key);draw();update();if(typed.length===text.length)finish()});
